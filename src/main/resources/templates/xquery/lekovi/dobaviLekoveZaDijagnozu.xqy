...
where $pc/@id = "http://www.zis.rs/zdravstveni_kartoni/id1" return $pc/@upozorenje

for $lek in fn:doc("/db/rs/zis/lekovi.xml")/lekovi:lekovi/lek:lek
where $lek/@aktivan = "true" and $lek/lek:dijagnoza = "F411" and fn:not(functx:is-value-in-sequence($lek/lek:naziv, tokenize(fn:data($upozorenje), '-')) )
return $lek
